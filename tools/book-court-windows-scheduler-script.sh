...
output_file="C:/Users/sivad/Desktop/Spruce/spruce_court_book_$(date +%d-%m-%Y).txt"

echo "Spruce Tennis Court Reservation" | tee -a "$output_file"
echo "Current time: $(date)" | tee -a "$output_file"
...
echo "First PHPSESSID: $phpsessid" | tee -a "$output_file"
...
do
  ...
  if [ $? -eq 0 ]; then
     echo "Booking done for $court" | tee -a "$output_file"
     break
  else
     echo "Booking failed for $court, trying another court..." | tee -a "$output_file"
  fi
  echo | tee -a "$output_file"
done